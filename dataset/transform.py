...
def crop(img, img_a, mask, size, ignore_value=255):
    w, h = img.size
...
def hflip(img, img_a, mask, p=0.5):
    if random.random() < p:
...
def random_grayscale(img, img_a, p=0.2):
    import torchvision.transforms.functional as TF
    if random.random() < p:
        # 如果命中概率，则全部转为灰度图
        # num_output_channels=3 是为了保持通道数不变（R=G=B），方便后续输入网络
        img = TF.to_grayscale(img, num_output_channels=3)
        img_a = TF.to_grayscale(img_a, num_output_channels=3)

    return img, img_a

def blur(img, img_a, p=0.5):
    if random.random() < p:
